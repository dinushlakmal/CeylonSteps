...
# We need to replace the section from `// Standard Past/Upcoming Footprints Trail` to just before `// 3. Add 3D Photo Pin Markers for each Footprint`
sed -i '/\/\/ Standard Past\/Upcoming Footprints Trail/,/\/\/ 3. Add 3D Photo Pin Markers for each Footprint/c\
$(cat patch.kt)\
                    \/\/ 3. Add 3D Photo Pin Markers for each Footprint' app/src/main/java/com/example/ui/components/OsmMapView.kt
